...
static uint8_t readByte() {
    ...
    gpio_set_level(RD, 0);
    uint32_t dat = (gpio_get_level(D0) << 0) | (gpio_get_level(D1) << 1) |
                   (gpio_get_level(D2) << 2) | (gpio_get_level(D3) << 3) |
                   (gpio_get_level(D4) << 4) | (gpio_get_level(D5) << 5) |
                   (gpio_get_level(D6) << 6) | (gpio_get_level(D7) << 7);
    gpio_set_level(RD, 1);
