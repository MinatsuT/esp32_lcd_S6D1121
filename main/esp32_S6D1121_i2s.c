...
static void initGPIO() {
    // Route GPIO_OUT_REG -> GPIO matrix -> IO_MUX -> pads.
...
static void initI2SLcdMode() {
    /* Route I2S peripheral outputs -> GPIO matrix */
    ...
    gpio_matrix_out(WR, I2S0O_WS_OUT_IDX, 1,
                    0); // WR (Write-strobe in 8080 mode, Active-low)
    gpio_matrix_out(RS, I2S0O_DATA_OUT16_IDX, 0,
                    0); // RS (Command/Data select: 0=CMD, 1=DATA)
...
// Send a buffer to the LCD using DMA. Not very intelligent, sends only one
// buffer per time, doesn't
// chain descriptors.
// We should end up here after an lcdFlush, with the I2S peripheral reset and
// clean but configured for
// FIFO operation.
static void sendBufDma(uint16_t *buf, int len) {
    // Fill DMA descriptor
    ...
    I2S0.lc_conf.val = I2S_OUT_DATA_BURST_EN | I2S_CHECK_OWNER |
                       I2S_OUT_EOF_MODE | I2S_OUTDSCR_BURST_EN |
...
static int icos(int i) { return isin(i + 256); }
...
#if 0
    for(int i=0;i<256;i++) {
	printf("func_in_sel_cfg[%d] func_sel=%d sig_in_sel=%d\n", i,
	       GPIO.func_in_sel_cfg[i].func_sel,
	       GPIO.func_in_sel_cfg[i].sig_in_sel);
    }
    printf("----------------------------------------------------------------------\n");
